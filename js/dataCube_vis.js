...
$(function() {
    ...
    /* getDatasets()
     .then((datasets)=> {
         console.log("Todo legal", datasets);
     })
     */
    configStaticUI();
...
function createAreaChartLinearX(timeFormatString) {
    ...
    var maxVal = d3.max(_observations, function(d) {
        return parseFloat(d[CKEYS.measObs]);
    });
    ...
    //------------------------------------------------------------------------------
    //Find the min value
    var minVal = d3.min(_observations, function(d) {
        return parseFloat(d[CKEYS.measObs]);
    });
...
function createBarChartOrdinalX() {
    ...
    var maxXaxisLabelLength = d3.max(_observations, function(d) {
        return (d[CKEYS.dimLabel].length)
    });
    ...
    if (maxXaxisLabelLength > barConfig.xLabelLengthTresholdForRotation) {
        ...
        var tempXScale = d3.scaleBand()
            ...
            .domain(_observations.map(function(d) {
                return d[CKEYS.dimLabel];
            }))
        ...
        if (xAxisHeight > barConfig.bottomMarginMax) {
            xAxisHeight = barConfig.bottomMarginMax;
        }
        margin = {
